...
type Props = {
  elementKey: CharityTextElementKey;
  content: CharitySiteContent;
  onChange: (next: CharitySiteContent) => void;
};
...
const COLOR_PRESETS = [
  ...
  { value: "#2F5D8C", label: "כחול" },
  { value: "#E53935", label: "אדום" },
  { value: "#F4C542", label: "צהוב" },
  { value: "#22C55E", label: "ירוק" },
  { value: "#A07820", label: "זהב כהה" },
  { value: "#A1A1AA", label: "אפור" },
];

export function CharityTextStylePanel({
  elementKey,
  content,
  onChange,
}: Props) {
  const meta = CHARITY_TEXT_ELEMENTS[elementKey];
  const text = getCharityText(content, elementKey);
  const style = getCharityStyle(content, elementKey);

  function updateText(value: string) {
    onChange(setCharityText(content, elementKey, value));
  }
  function updateStyle(patch: Partial<TextStyle>) {
    onChange(setCharityStyle(content, elementKey, { ...style, ...patch }));
  }
  function clearOverride<K extends keyof TextStyle>(field: K) {
    const next = { ...style };
    delete next[field];
    onChange(setCharityStyle(content, elementKey, next));
  }
  function resetAll() {
    onChange(setCharityStyle(content, elementKey, {}));
  }
